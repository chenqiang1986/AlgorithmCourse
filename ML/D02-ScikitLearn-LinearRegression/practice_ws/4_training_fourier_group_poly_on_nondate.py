...
def main():
    ...
    polynomialize = ColumnTransformer(
        [
            ("ployCol", PolynomialFeatures(degree=1, include_bias=False), numeric_features)
        ],
    ...
    polynomialize.fit(X_train)
    poly_features = polynomialize.get_feature_names_out().tolist()
    poly_features = [f 
                     for f in poly_features 
                     if any(n in f for n in numeric_features)
    ]
    
    preprocessor = ColumnTransformer(
        [
            ("cat", OneHotEncoder(handle_unknown="ignore"), categorical_features),
            ("group", GroupInteraction("Store"), ["Store"] + poly_features),         
            ("fourier", FourierFeatures(degree=50), fourier_features),
        ],

    )

    regressor = TransformedTargetRegressor(
        regressor=Ridge(alpha=1e-6), func=np.log, inverse_func=np.exp
    )

    model = Pipeline(
        [
            ("polynomialize", polynomialize),
