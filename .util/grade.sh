...
run_cpp_target() {
    ...
    while :
    do
       if [ ! -f .test_cases/$non_ext_base_name/$i.in ]; then
           echo "This is the end of the loop"
           break
       fi
       
       echo "   - Run Test Case $i"
       timeout 10s build/$non_ext_base_name < .test_cases/$non_ext_base_name/$i.in > build/$non_ext_base_name.out
       execute_exit_code=$?
       echo "   - Exit code $execute_exit_code"
       
       diff -q .test_cases/$non_ext_base_name/$i.out build/$non_ext_base_name.out >/dev/null
       diff_exit_code=$?
       ...
       then
           ...
           diff .test_cases/$non_ext_base_name/$i.out build/$non_ext_base_name.out
       else
