...
```text
arrangements of n items with A and B forced adjacent
  = (arrangements of (n - 1) "items", where the glued block counts as 1 item)
    * (arrangements inside the block)
  = (n - 1)! * 2!
```
...
```text
arrangements of n items with A and B forced non-adjacent
  = (arrangements of the other n - 2 items) * (ways to place A and B into 2 different gaps, in order)
  = (n - 2)! * P(n - 1, 2)
```
...
```text
arrangements with A, B non-adjacent = n! - (n - 1)! * 2!
```
...
**Method 2 — complement.** Total unrestricted arrangements: `6! = 720`. Arrangements with Cara and Dan forced together: `5! * 2! = 240`.
...
Total unrestricted arrangements: `7! = 5,040`. Arrangements with Lena and Omar forced together: `6! * 2! = 720 * 2 = 1,440`.
...
Arrange the other `6 - 3 = 3` people first: `3! = 6` ways, creating `3 + 1 = 4` gaps. Place the 3 friends into 3 of the 4 gaps (order matters, one friend per chosen gap): `P(4, 3) = 4 * 3 * 2 = 24`.
...
If you need to place more mutually non-adjacent items than there are available gaps, the answer is `0` — always check `(items to separate) <= (number of gaps)` before computing.
