...
If every box must get **at least 1** ball (positive integers only, requires $n \ge k$), first place 1 ball in every box to satisfy the minimum, then distribute the remaining $n - k$ balls with no restriction, using the same formula with $n - k$ balls and $k$ boxes:
